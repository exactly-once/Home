...
Assumptions: Previous posts intoduces how duplicates and message re-ordering can happen
---

## Introduction
(Delivery guarantees)[TODO: link-to-the-previous-post] of modern messaging infrastructures make it non-trivial to build roboust distributed systems. This post goes through problems that system builders have to tackle and shows how idempotency can help. Not some generic idempotency though but quite a special flavour. 
...
Let's extend our system with a second `LeaderBoard` endpoint that's responsible for storing number of target hits for the player. The endpoint processes `Hit` messages generated by the `ShootingRange`:
...
We didn't end-up with an unexpected state e.g. with an attempt resulting with miss when we expected a hit. It's far worse than that! We ended up in a state which indicates that two logically exclusive alternatives occured. A state which is simply corrupted.
...
(4) - both scenarios are indistiguishable from the perspective of any receiving endpoint
